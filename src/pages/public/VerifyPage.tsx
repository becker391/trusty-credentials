...
import { ClipboardPaste, Upload, ShieldCheck, FileText, X } from 'lucide-react';
...
export default function VerifyPage() {
  ...
  const handleFileSelect = (file: File) => {
    ...
    // Check file type
    const allowedTypes = ['.json', '.txt', '.csv', '.pdf', '.png', '.jpg', '.jpeg'];
    ...
    <div className="container mx-auto px-4 py-16">
      <div className="max-w-2xl mx-auto space-y-8">
        ...
        <div className="text-center space-y-3">
          <div className="mx-auto h-14 w-14 rounded-xl bg-accent/10 flex items-center justify-center">
            <ShieldCheck className="h-7 w-7 text-accent" />
          </div>
          <h1 className="text-3xl md:text-4xl font-bold">Verify a Credential</h1>
          <p className="text-muted-foreground max-w-lg mx-auto">
            Instantly verify the authenticity of any academic credential by entering its unique hash or uploading a credential file. No account required.
          </p>
        </div>
                ...
                {!selectedFile ? (
                  ...
                  >
                    ...
                    <input
                      ...
                      type="file"
                      className="hidden"
                      accept=".json,.txt,.csv,.pdf,.png,.jpg,.jpeg"
                      onChange={(e) => {
        ...
        {history.length > 0 && (
          <Card className="glow-card">
            <CardHeader><CardTitle className="text-sm">Verification History</CardTitle></CardHeader>
            <CardContent>
              <div className="space-y-2">
                {history.map((h, i) => (
